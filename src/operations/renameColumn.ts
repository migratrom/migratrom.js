...
export function renameColumn(
	...
	const alterSql = `ALTER TABLE ${qTable} RENAME COLUMN ${dialect.quoteIdent(from)} TO ${dialect.quoteIdent(to)}`;
	...
	return {
		...
		precheck: [
			check(
				...
				dialect.columnExistsSql(schema, table, from, false),
			),
			check(
				`ensure column "${to}" does not exist on "${table}"`,
				dialect.columnExistsSql(schema, table, to, true),
			),
		],
		...
		postcheck: [
			check(
				...
				dialect.columnExistsSql(schema, table, to, false),
			),
		],
	};
}
